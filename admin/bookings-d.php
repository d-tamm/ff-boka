...
function showCat( Category $cat, User $user ) : void {
    if ( $cat->showFor( $user, FFBoka::ACCESS_CONFIRM ) ) {
        ...
        echo "<i title='Visa kategorin' style='display:" . ( isset($_COOKIE[ 'bookingAdminShowCat' . $cat->id ]) ? "none" : "inline" ) . ";' id='cat-opener-{$cat->id}' class='far fa-plus-square' onClick='showHideCat({$cat->id}, 1);'></i>";
        echo "<i title='Göm kategorin' style='display:" . ( isset($_COOKIE[ 'bookingAdminShowCat' . $cat->id ]) ? "inline" : "none" ) . ";' id='cat-closer-{$cat->id}' class='far fa-minus-square' onClick='showHideCat({$cat->id}, 0);'></i></span> ";
        ...
        echo "</h2>\n";
        echo "<div style='display:" . ( isset($_COOKIE[ "bookingAdminShowCat$cat->id" ]) ? "block" : "none" ) . ";' id='cat-table-{$cat->id}'>\n";
