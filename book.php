...
<div data-role="page" id="page_book1">
    ...
    <?php
    foreach ($section->getMainCategories() as $cat) { // TODO: recurse into sub-categories
        if ($cat->showFor($currentUser)) { ?>
            <div data-role='collapsible' data-inset='false'>
                <h3><div class="cat-list-img"><?= embedImage($cat->thumb) ?></div><?= $cat->caption ?></h3>
                <?= $cat->bookingMsg ? "<p>{$cat->bookingMsg}</p>" : "" ?>
                ...
                foreach ($cat->items() as $item) {
                    if ($item->active) {
                        echo "<li id='book-item-{$item->id}'><a href=\"javascript:toggleItem({$item->id});\">" .
                            ...
                            "<h4>{$item->caption}</h4>". $item->bookingBar(strtotime("this monday")) . "</a>" .
